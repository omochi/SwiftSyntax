...
public class FloatLiteralExprSyntax: ExprSyntax {
  ...
  override func validate() {
      if isMissing { return }
    precondition(raw.layout.count == 2)
    let _sign = raw[Cursor.prefixOperatorToken]
    guard let _signTokenKind = _sign.tokenKind else {
      fatalError("expected token child, got \(_sign.kind)")
    }
    precondition([.prefixOperator].contains(_signTokenKind),
      "expected one of [.prefixOperator] for 'sign' " + 
  ...
  public var sign: TokenSyntax? {
    guard raw[Cursor.sign].isPresent else { return nil }
    return TokenSyntax(root: _root,
      data: data.cachedChild(at: Cursor.sign))
  }

  public func withSign(
...
public typealias StructMembersSyntax = SyntaxCollection<DeclSyntax>
public class DeclModifierSyntax: Syntax {
  ...
  override func validate() {
    ...
    let _argument = raw[Cursor.identifierToken]
    guard let _argumentTokenKind = _argument.tokenKind else {
      fatalError("expected token child, got \(_argument.kind)")
    }
    precondition([.identifier].contains(_argumentTokenKind),
      "expected one of [.identifier] for 'argument' " + 
      "in node of kind declModifier")
    let _rightParen = raw[Cursor.rightParenToken]
    guard let _rightParenTokenKind = _rightParen.tokenKind else {
      fatalError("expected token child, got \(_rightParen.kind)")
  ...
  public var leftParen: TokenSyntax? {
    guard raw[Cursor.leftParen].isPresent else { return nil }
    return TokenSyntax(root: _root,
      data: data.cachedChild(at: Cursor.leftParen))
  }
  ...
  public var argument: TokenSyntax? {
    guard raw[Cursor.argument].isPresent else { return nil }
    return TokenSyntax(root: _root,
      data: data.cachedChild(at: Cursor.argument))
  }
  ...
  public var rightParen: TokenSyntax? {
    guard raw[Cursor.rightParen].isPresent else { return nil }
    return TokenSyntax(root: _root,
      data: data.cachedChild(at: Cursor.rightParen))
  }
...
public class StructDeclSyntax: DeclSyntax {
  ...
  override func validate() {
    ...
    precondition(_members.kind == .structMembers,
                 "expected child of kind .structMembers, " +
                 "got \(_members.kind)")
  ...
  public var members: StructMembersSyntax {
    return StructMembersSyntax(root: _root,
      data: data.cachedChild(at: Cursor.members))
  }

  public func addDecl(_ elt: DeclSyntax) -> StructDeclSyntax {
  ...
  public func withMembers(
    _ newStructMembersSyntax: StructMembersSyntax?) -> StructDeclSyntax {
    let raw = newStructMembersSyntax?.raw ?? RawSyntax.missing(.structMembers)
    let (root, newData) = data.replacingChild(raw,
                                              at: Cursor.members)
    return StructDeclSyntax(root: root, data: newData)
  }
...
public class SameTypeRequirementSyntax: Syntax {
  ...
  override func validate() {
    ...
    precondition(_leftTypeIdentifier.kind == .typeIdentifier,
                 "expected child of kind .typeIdentifier, " +
  ...
  public var leftTypeIdentifier: TypeIdentifierSyntax {
    return TypeIdentifierSyntax(root: _root,
      data: data.cachedChild(at: Cursor.leftTypeIdentifier))
  }

  public func withLeftTypeIdentifier(
    _ newTypeIdentifierSyntax: TypeIdentifierSyntax?) -> SameTypeRequirementSyntax {
    let raw = newTypeIdentifierSyntax?.raw ?? RawSyntax.missing(.typeIdentifier)
    let (root, newData) = data.replacingChild(raw,
  ...
  public var rightTypeIdentifier: TypeIdentifierSyntax {
    return TypeIdentifierSyntax(root: _root,
...
public class GenericParameterSyntax: Syntax {
  ...
  override func validate() {
      if isMissing { return }
    precondition(raw.layout.count == 4)
    let _typeIdentifier = raw[Cursor.typeIdentifier]
    precondition(_typeIdentifier.kind == .typeIdentifier,
                 "expected child of kind .typeIdentifier, " +
                 "got \(_typeIdentifier.kind)")
    let _colon = raw[Cursor.colonToken]
    guard let _colonTokenKind = _colon.tokenKind else {
      fatalError("expected token child, got \(_colon.kind)")
    }
  ...
  public var typeIdentifier: TypeIdentifierSyntax {
    return TypeIdentifierSyntax(root: _root,
      data: data.cachedChild(at: Cursor.typeIdentifier))
  }

  public func withTypeIdentifier(
    _ newTypeIdentifierSyntax: TypeIdentifierSyntax?) -> GenericParameterSyntax {
    let raw = newTypeIdentifierSyntax?.raw ?? RawSyntax.missing(.typeIdentifier)
    let (root, newData) = data.replacingChild(raw,
...
public class ConformanceRequirementSyntax: Syntax {
  ...
  public func withLeftTypeIdentifier(
    _ newTypeIdentifierSyntax: TypeIdentifierSyntax?) -> ConformanceRequirementSyntax {
    let raw = newTypeIdentifierSyntax?.raw ?? RawSyntax.missing(.typeIdentifier)
    let (root, newData) = data.replacingChild(raw,
                                              at: Cursor.leftTypeIdentifier)
...
public class ProtocolCompositionElementSyntax: Syntax {
  ...
  public func withProtocolType(
    _ newTypeIdentifierSyntax: TypeIdentifierSyntax?) -> ProtocolCompositionElementSyntax {
    let raw = newTypeIdentifierSyntax?.raw ?? RawSyntax.missing(.typeIdentifier)
    let (root, newData) = data.replacingChild(raw,
...
public class TypeIdentifierSyntax: TypeSyntax {
  enum Cursor: Int {
    case typeName
    case genericArgumentClause
    case period
  ...
  override func validate() {
    ...
    guard let _typeNameTokenKind = _typeName.tokenKind else {
      fatalError("expected token child, got \(_typeName.kind)")
    }
    precondition([.identifier].contains(_typeNameTokenKind),
      "expected one of [.identifier] for 'typeName' " + 
      "in node of kind typeIdentifier")
    let _genericArgumentClause = raw[Cursor.genericArgumentClause]
    precondition(_genericArgumentClause.kind == .genericArgumentClause,
                 "expected child of kind .genericArgumentClause, " +
                 "got \(_genericArgumentClause.kind)")
  ...
  public func withTypeName(
    _ newTokenSyntax: TokenSyntax?) -> TypeIdentifierSyntax {
    let raw = newTokenSyntax?.raw ?? RawSyntax.missingToken(.identifier(""))
    let (root, newData) = data.replacingChild(raw,
                                              at: Cursor.typeName)
  ...
  public func withGenericArgumentClause(
    _ newGenericArgumentClauseSyntax: GenericArgumentClauseSyntax?) -> TypeIdentifierSyntax {
    let raw = newGenericArgumentClauseSyntax?.raw ?? RawSyntax.missing(.genericArgumentClause)
    let (root, newData) = data.replacingChild(raw,
                                              at: Cursor.genericArgumentClause)
    return TypeIdentifierSyntax(root: root, data: newData)
  ...
  public var period: TokenSyntax? {
    guard raw[Cursor.period].isPresent else { return nil }
    return TokenSyntax(root: _root,
      data: data.cachedChild(at: Cursor.period))
  }
  ...
  public var typeIdentifier: TypeIdentifierSyntax? {
    guard raw[Cursor.typeIdentifier].isPresent else { return nil }
    return TypeIdentifierSyntax(root: _root,
      data: data.cachedChild(at: Cursor.typeIdentifier))
  }

  public func withTypeIdentifier(
    _ newTypeIdentifierSyntax: TypeIdentifierSyntax?) -> TypeIdentifierSyntax {
    let raw = newTypeIdentifierSyntax?.raw ?? RawSyntax.missing(.typeIdentifier)
    let (root, newData) = data.replacingChild(raw,
                                              at: Cursor.typeIdentifier)
    return TypeIdentifierSyntax(root: root, data: newData)
...
public class EnumCasePatternSyntax: PatternSyntax {
  ...
  public func withTypeIdentifier(
    _ newTypeIdentifierSyntax: TypeIdentifierSyntax?) -> EnumCasePatternSyntax {
    let raw = newTypeIdentifierSyntax?.raw ?? RawSyntax.missing(.typeIdentifier)
    let (root, newData) = data.replacingChild(raw,
                                              at: Cursor.typeIdentifier)
